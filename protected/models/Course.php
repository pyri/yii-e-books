...
class Course extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, type_of_publication, org, source, user_id, username, author, publish, description', 'required'),
			array('user_id, publish', 'numerical', 'integerOnly'=>true),
			array('title, username, author', 'length', 'max'=>100),
			array('type_of_publication', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, type_of_publication, org, source, user_id, username, author, publish, description', 'safe', 'on'=>'search'),
		);
	}
